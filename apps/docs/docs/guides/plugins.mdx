...
Add a new effect. Example: `setPlaceholder`:

```ts
import type { EffectPlugin } from "formwright/core";

const setPlaceholderEffect: EffectPlugin = {
  kind: "effect",
  identity: { name: "setPlaceholder", version: "1" },
  effectType: "setPlaceholder",

  apply({ effect, fieldStateMutations }) {
    // effect is { type: "setPlaceholder", target: "fieldPath", value: "Enter..." }
    fieldStateMutations.push({
      path: effect.target as string,
      mutation: (state) => ({ ...state, placeholder: effect.value }),
    });
...
const phoneFieldPlugin: FieldPlugin = {
  ...
  normalize(input) {
    return {
      rendererKey: "phone",
      validationRules: input.definition.required
  ...
  getValidationPlan(input) {
    return input.definition.required
