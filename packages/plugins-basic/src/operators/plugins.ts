...
function evaluateExpression(expr: unknown, values: Record<string, unknown>): unknown {
  ...
  if ("eq" in node && Array.isArray(node.eq) && node.eq.length === 2) {
    return evaluateExpression(node.eq[0], values) === node.eq[1];
  }
  if ("neq" in node && Array.isArray(node.neq) && node.neq.length === 2) {
    return evaluateExpression(node.neq[0], values) !== node.neq[1];
  }
  if ("and" in node && Array.isArray(node.and)) {
    return node.and.every((entry) => Boolean(evaluateExpression(entry, values)));
  }
  if ("or" in node && Array.isArray(node.or)) {
    return node.or.some((entry) => Boolean(evaluateExpression(entry, values)));
  }
  if ("not" in node) {
    return !Boolean(evaluateExpression(node.not, values));
  }
  ...
  if ("gt" in node && Array.isArray(node.gt) && node.gt.length === 2) {
    return Number(evaluateExpression(node.gt[0], values)) > Number(node.gt[1]);
  }
  if ("gte" in node && Array.isArray(node.gte) && node.gte.length === 2) {
    return Number(evaluateExpression(node.gte[0], values)) >= Number(node.gte[1]);
  }
  if ("lt" in node && Array.isArray(node.lt) && node.lt.length === 2) {
    return Number(evaluateExpression(node.lt[0], values)) < Number(node.lt[1]);
  }
  if ("lte" in node && Array.isArray(node.lte) && node.lte.length === 2) {
    return Number(evaluateExpression(node.lte[0], values)) <= Number(node.lte[1]);
  }
  if ("in" in node && Array.isArray(node.in) && node.in.length === 2 && Array.isArray(node.in[1])) {
    return node.in[1].includes(evaluateExpression(node.in[0], values));
  }
...
function makeOperator(operatorType: string): OperatorPlugin {
  return {
    ...
    operatorType,
    evaluate(input) {
      return evaluateExpression(input.expression, input.values);
    },
  };
}
